...
let todos = []; // 할 일 목록을 저장할 배열을 초기화

//? 1. 할 일 추가
// : 고육 ID, 내용, 완료 상태를 가지는 객체를 생성
// >> 배열에 추가
...
todos = todos.map((todo) => {
  // 순회되는 할 일(todo)id와 매개변수로 전달받은 id가 '일치한다면'
  if(todo.id === id){
  // 해당 할 일의 완료 상태를 토글시키고 새로운 객체로 반환
  // >> 스프레드 연산자(...)
  
  // let arr1 = [1, 2, 3];
  // let arr2 = [...ar1]; >> arr1의 요소만 가져와서 새로운 배열을 생성(메모리 주소가 다름)
  ...
  return todo; // id가 일치하지 않는 데이터는 기본 객체를 그대로 반환
});
displayTodos();
}

//? 할 일을 삭제하는 함수
// : 주어진 id를 가진 할 일을 toods 배열에서 제거
...
function displayTodos() {
  ...
  todos.forEach(todo => {
    ...
    // >> 조건식 중에 연산자를 활용하는 '삼황 연산자' (조건식 ? 참 : 거짓)
