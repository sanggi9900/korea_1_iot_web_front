// export const tmp = '';
{
  ...
  const fetchUsers = async (page: number, limit: number = 3): Promise<Users> => {
    ...
    try {
      ...
      // : _limtt옵션 - 데이터를 조회할 때 한 번에 가져올 수 있는 항목의 최대 수를 지정
      
      // >> _page 옵션을 사용하는 경우 기본적으로 각 페이지에 한개의 데이터가 반환
      const response = await fetch(`https://jsonplaceholder.typicode.com/users?_page=${page}&_limit=${limit}`);  // 시간이걸리는 작업을 기다리기 위해 await 사용
      // page에 1: 1, 2, 3
      // page에 2: 4, 5, 6
      // page에 3: ...
      ...
      console.error('Fetch error:', error);  // e
      return [];
    }
  };
  
  //# 각 데이터가 나열되 카드를 동적으로 생성하는 함수
  // 매개뱐수 : User타입(객체)의 데이터
...
// 매개변수: 사용자 배열 전달
  ...
  let currentPage = 1; // 변경되기때문 let 필수
  ...
  const loadPage = async (page: number): Promise<void> => {
    ...
    // 페이지 번호도 수정
    updatePageInfo();  
    // ctrl 누르고 함수명 클릭시 해당 정의된 함수로 이동한다
  };
