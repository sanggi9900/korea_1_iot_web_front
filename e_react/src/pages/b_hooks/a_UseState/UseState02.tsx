...
const person2 = {...perosn, name: '이도경', hobby: '독서'};
const person3 = {...perosn, ...person1};
...
//    : 상태 설정 함수를 사용 + 새로운 참저값에 데이터를 담아 전달
...
export default function UseState02() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    // HTMLElement : 사용하면 다 포함되지만 각각의 value를 찾기 힘들다
    // let inpuText = e.target.value;
    // setInputValue(inpuText)
  ...
  const handleLoginChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // - 아이디와 비밀번호 input 창을 변경할 수 있는 핸들러
    // e.target
  ...
  return (
    <div>
      ...
      */}
      ...
      {/* 
      >> onChange 이벤트
       : 사용자ㅏ 요소에 변화를 일으킬 때 마다 발생하는 이벤트
